...
use crate::ray::Ray;
use crate::shape::{CanIntersect, hit, Shape};
...
pub fn chapter6_render_shaded_sphere(resolution: usize) {
    ...
    println!(
        "Rendering sphere at {} x {} resolution",
    ...
    let mut last_progress = 0; // last progress reported
    for x in 0..canvas_dimensions {

        // print progress once in a while
        let current_progress = 100 * x / canvas_dimensions;
        let progress_interval = 10; //print progress every ~10%
        if current_progress > last_progress + 10 {
            println!("\t{}% done", current_progress);
    ...
    let filename = "ch6_shaded_sphere";
